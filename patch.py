...
with open('client/src/components/TicketDetail.tsx', 'r', encoding='utf8') as f:
    content = f.read()
...
'''
content = re.sub(r'interface Ticket \{[\s\S]*?\n\}', interface_code, content)
...
'''
content = re.sub(r'<div className="flex justify-between items-end mb-6 border-b pb-4 border-zenPrimary">[\s\S]*?</div>\s*</div>', resolved_ui, content)
...
with open('client/src/components/TicketDetail.tsx', 'w', encoding='utf8') as f:
    f.write(content)
