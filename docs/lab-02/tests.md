...
| API-05 | ตรวจสอบไฟล์ | อัปโหลดไฟล์แนบขนาดเกิน 5MB (เช่น 5.1MB) | 400 Bad Request; อัปโหลดไม่สำเร็จ | `attachments.api.test.ts` | Pending |
...
| API-07 | ตรวจสอบไฟล์ | อัปโหลดไฟล์แนบเกินโควต้า (อัปโหลด 6 ไฟล์พร้อมกัน) | 400 Bad Request; อัปโหลดไม่สำเร็จ | `attachments.api.test.ts` | Pending |
| API-08 | ความปลอดภัย | เรียก API โดยไม่ส่ง Header X-Requester-Id | 401 Unauthorized | `tickets.api.test.ts` | Pending |
| UI-03a | Accessibility | ใช้คีย์บอร์ด (Tab, Enter) นำทางในหน้าฟอร์ม | โฟกัสเปลี่ยนตำแหน่งถูกต้องและกดปุ่มได้ | `responsive.spec.ts` | Pending |
| UI-03b | Responsive | ตรวจสอบการแปลง Table เป็น Card บนหน้าจอมือถือ | แสดงผลเป็นการ์ดเรียงซ้อนกันถูกต้อง | `responsive.spec.ts` | Pending |
